...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`cn-ben` /*!40100 DEFAULT CHARACTER SET utf8mb4 */;

USE `cn-ben`;

/*Table structure for table `t_notify_log` */

DROP TABLE IF EXISTS `t_notify_log`;

CREATE TABLE `t_notify_log` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `notify_id` varchar(32) DEFAULT NULL COMMENT '通知记录ID',
  `http_status` int(10) unsigned DEFAULT NULL COMMENT '通知-Http请求状态码',
  `http_response` varchar(256) DEFAULT NULL COMMENT '通知-Http请求响应',
  `http_exception` varchar(256) DEFAULT NULL COMMENT '通知-Http请求异常信息(如：timeout)',
  `business_name` varchar(32) DEFAULT NULL COMMENT '业务名称',
  `business_id` varchar(32) DEFAULT NULL COMMENT '业务方记录ID',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `index_notify_id` (`notify_id`),
  KEY `index_business_id` (`business_id`),
  KEY `index_create_time` (`create_time`)
) ENGINE=InnoDB AUTO_INCREMENT=216674 DEFAULT CHARSET=utf8mb4;

/*Data for the table `t_notify_log` */

/*Table structure for table `t_notify_record` */

DROP TABLE IF EXISTS `t_notify_record`;

CREATE TABLE `t_notify_record` (
  `id` varchar(32) NOT NULL,
  `notify_url` varchar(256) DEFAULT NULL COMMENT '通知-请求地址',
  `notify_method` tinyint(2) DEFAULT NULL COMMENT '通知-请求方式 0:GET, 1:POST, 2:HEAD, 3:OPTIONS, 4:PUT, 5:DELETE, 6:TRACE, 7:PATCH',
  `notify_header` varchar(256) DEFAULT NULL COMMENT '通知-请求头',
  `notify_param` longtext NOT NULL COMMENT '通知-请求参数',
  `notify_param_type` tinyint(2) DEFAULT NULL COMMENT '通知-请求参数类型 0:FORM 1:BODY',
  `notify_timeout` smallint(6) unsigned DEFAULT '5000' COMMENT '通知-请求超时时长，单位：毫秒',
  `success_flag` varchar(128) DEFAULT NULL COMMENT '通知-成功响应标识（http请求响应包含此内容时即通知成功）（设为空或空字符串时，http响应码为2xx即通知成功）',
  `notify_times` smallint(5) unsigned DEFAULT '0' COMMENT '已通知次数',
  `notify_status` tinyint(2) unsigned DEFAULT '0' COMMENT '通知状态 0待通知 1通知成功 2已通知(业务方处理失败) 3Http请求异常 4通知失败（超过通知次数）',
  `business_name` varchar(32) DEFAULT NULL COMMENT '业务名称',
  `business_id` varchar(32) DEFAULT NULL COMMENT '业务方记录ID',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  ...
  KEY `index_business_id` (`business_id`),
  KEY `index_create_time` (`create_time`),
  KEY `index_notify_status` (`notify_status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

/*Data for the table `t_notify_record` */
